...
def send_http(session, method, url, *,
    ...
    if method.lower() == "post":
        ...
        logger.info("url: {} method: post headers: {} data: {}".format(url, json.dumps(headers), data))
    else:
        _id = md5(url)
        logger.info("url: {} method: {} headers: {}".format(url, method, json.dumps(headers)))
    ...
    while attempt != 0:
        try:
            ...
            cacheDB.save(data={"_id": _id, "response": pickle.dumps(response)}, tname=tname)
